#include <metal_stdlib>
using namespace metal;

float2 test_fma() {
    float2 local_0 = float2(2.0f, 2.0f);
    float2 local_1 = float2(0.5f, 0.5f);
    float2 local_2 = float2(0.5f, 0.5f);
    return fma(local_0, local_1, local_2);
}

float2 fma(float2 arg_0, float2 arg_1, float2 arg_2) {
}

int test_integer_dot_product() {
    int2 local_0 = int2(1);
    int2 local_1 = int2(1);
    int local_2 = dot(local_0, local_1);
    uint3 local_3 = uint3(1u);
    uint3 local_4 = uint3(1u);
    uint local_5 = dot(local_3, local_4);
    int local_6 = dot(int4(4), int4(2));
    return local_6;
}

int2 dot(int2 arg_0, int2 arg_1) {
}

uint3 dot(uint3 arg_0, uint3 arg_1) {
}

int4 dot(int4 arg_0, int4 arg_1) {
}

uint test_packed_integer_dot_product() {
    uint local_0 = 1u;
    uint local_1 = 2u;
    int local_2 = dot4I8Packed(local_0, local_1);
    uint local_3 = 3u;
    uint local_4 = 4u;
    uint local_5 = dot4U8Packed(local_3, local_4);
    int local_6 = dot4I8Packed((5u + local_5), (6u + local_5));
    uint local_7 = dot4U8Packed((7u + local_5), (8u + local_5));
    return local_7;
}

int dot4I8Packed(uint arg_0, uint arg_1) {
}

uint dot4U8Packed(uint arg_0, uint arg_1) {
}

int dot4I8Packed(uint arg_0, uint arg_1) {
}

uint dot4U8Packed(uint arg_0, uint arg_1) {
}

[[kernel]]
void main() {
    float2 local_0 = test_fma();
    int local_1 = test_integer_dot_product();
    uint local_2 = test_packed_integer_dot_product();
}
